...
if ( ! function_exists( 'aiosp_log' ) ) {
	// preferred usage is using do_action and not calling the function directly.
	add_action( 'aiosp_log', 'aiosp_log', 10, 5 );

	// log to the database and into file.
	function aiosp_log( $log_msg, $log_type = 'debug', $file = null, $line = null, $force = false ) {
		global $aioseop_options;
		// keep only the last N logs and for how long
		static $log_length, $log_expiry;

		if ( is_null( $log_length ) ) {
			$log_length = apply_filters( 'aioseop_log_length', 100 );
		}
		if ( is_null( $log_expiry ) ) {
			$log_expiry = apply_filters( 'aioseop_log_expiry_minutes', 5 );
		}
		...
		$logs = array_slice( $logs, 0 - $log_length );
		set_transient( 'aiosp_event_log', $logs, $log_expiry * MINUTE_IN_SECONDS );
...
if ( ! function_exists( 'aiosp_log_show' ) ) {
	function aiosp_log_show() {
		$logs = get_transient( 'aiosp_event_log' );
?>
<div id="aiosp-logs">
	<div id="aiosp-log-actions">
		...
		<?php foreach ( array( 'error', 'warn', 'info', 'debug' ) as $type ) { ?>
