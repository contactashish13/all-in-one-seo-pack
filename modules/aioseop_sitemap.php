	...
	class All_in_One_SEO_Pack_Sitemap extends All_in_One_SEO_Pack_Module {
		...
		function __construct() {
			if ( get_class( $this ) === 'All_in_One_SEO_Pack_Sitemap' ) { // Set this up only when instantiated as this class.
				...
				$this->extra_sitemaps = apply_filters( $this->prefix . 'extra', array() );
			}
			parent::__construct();
		...
		function get_rewrite_rules() {
			...
			return apply_filters( $this->prefix . 'rewrite_rules', $sitemap_rules, $this->options );
		}
			...
			if ( $this->options[ "{$this->prefix}indexes" ] ) {
				...
				if ( 'root' === $sitemap_type ) {
					...
					if ( is_array( $this->extra_sitemaps ) && in_array( $sitemap_type, $this->extra_sitemaps ) ) {
						$sitemap_data = apply_filters( $this->prefix . 'custom_' . $sitemap_type, $sitemap_data, $page, $this->options, $this );
					}
				}
		...
		function get_sitemap_index_filenames() {
			...
			$files  = apply_filters( 'aioseop_sitemap_index_filenames', $files, $prefix, $suffix, $this->options, $this );
		...
		function get_simple_sitemap() {
			...
			if ( is_array( $this->extra_sitemaps ) ) {
				foreach ( $this->extra_sitemaps as $sitemap_type ) {
					$sitemap_data = array();
					$sitemap_data = apply_filters( $this->prefix . 'custom_' . $sitemap_type, $sitemap_data, 0, $this->options, $this );
		 ...
		 * @return array
		 */
		public function get_prio_from_posts( $posts, $prio_override = false, $freq_override = false, $linkfunc = 'get_permalink' ) {
			$prio = array();
			$args = array(
		...
		/**
		 * Returns the max posts that can be added per sitemap.
		 */
