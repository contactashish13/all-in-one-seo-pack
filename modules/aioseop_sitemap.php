	...
	class All_in_One_SEO_Pack_Sitemap extends All_in_One_SEO_Pack_Module {
		...
		private function init_module() {
			...
			$this->help_text = array(
				...
				'excl_taxonomies' => __( 'Entries from these taxonomies will be excluded from the sitemap.', 'all-in-one-seo-pack' ),
				'excl_categories' => __( 'Entries from these specific categories/terms will be excluded from the sitemap. If left blank, entries from the entire taxonomies selected above will be excluded.', 'all-in-one-seo-pack' ),
			...
			$this->help_anchors = array(
				...
				'excl_taxonomies' => '#excluded-items',
			...
			$excl_options = array(
				'excl_taxonomies' => array(
					'name'            => __( 'Excluded Taxonomies', 'all-in-one-seo-pack' ),
					'type'            => 'multicheckbox',
					'initial_options' => '',
					'class'           => 'aioseop-excl-taxonomy',
				),
				'excl_categories' => array(
					'name'            => __( 'Excluded Categories/Terms', 'all-in-one-seo-pack' ),
					'type'            => 'multiselect',
		...
		private function add_hooks() {
			add_action( 'after_doing_aioseop_updates', array( $this, 'after_upgrade_plugin' ) );
		...
		/**
		 * If a category has been selected previously, enable the Category taxonomy as well.
		 * If taxonomies have been selected, populate the term dropdown too.
		 */
		...
		function add_post_types() {
			...
			$this->default_options['excl_taxonomies']['initial_options'] = $taxonomy_titles;
			$this->default_options['excl_categories']['initial_options'] = array();

			add_filter( "{$this->prefix}display_options", array( $this, 'populate_exclude_options' ), 10, 1 );
		...
		function get_excluded_terms( $only_ids = false ) {
			...
			// now let's get the taxonomies that need to be excluded.
			if ( $this->option_isset( 'excl_taxonomies' ) ) {
				$taxonomy_specific = array_keys( $map );
				$taxonomies = $this->options[ $this->prefix . 'excl_taxonomies' ];
		...
		function get_tax_args( $page = 0 ) {
			$args = array();
			$exclude = $this->get_excluded_terms( true );
			if ( $exclude ) {
