jQuery( document ).on(
	'click', '.visibility-notice', function() {
...
jQuery( document ).on(
	'click', '.woo-upgrade-notice', function() {
...
jQuery( document ).on(
	'click', '.sitemap_max_urls_notice', function() {
...
function aioseop_ajax_edit_meta_form( post_id, meta, nonce ) {
	...
	jQuery( '#aioseop_' + meta + '_cancel_' + post_id ).click(
		function() {
			uform.html( element );
			uform.attr( "class", "aioseop_mpc_admin_meta_options" );
		}
	);
	jQuery( '#aioseop_' + meta + '_save_' + post_id ).click(
		function() {
...
function handle_post_meta( p, t, m, n ) {
	jQuery( "div#aioseop_" + m + "_" + p ).fadeOut(
		'fast', function() {
			...
			jQuery( "div#aioseop_" + m + "_" + p ).fadeIn(
				'fast', function() {
